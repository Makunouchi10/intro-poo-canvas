...
for (let i = 0; i < MaxCircles; i++) {
  ...
  let randomRadius = Math.floor(Math.random() * 15 + 25);
  ...
  // Colores aleatorios
  let lineColor = `rgb(${Math.floor(Math.random()*256)}, ${Math.floor(Math.random()*256)}, ${Math.floor(Math.random()*256)})`;
  ...
  let miCirculoMultiple = new Circle(
    ...
    randomY,
    randomRadius,
    lineColor,      // color de línea diferente
    i + 1,
    fillColor,      // color de fondo diferente
    "white"
  );
